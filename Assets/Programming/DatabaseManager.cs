...
public class DatabaseManager : MonoBehaviour {
    FPS_Network_Manager databaseNetworkManager;
    public DataTranslator databaseDataTranslator;
    ...
    [SerializeField] GameObject LoginUICanvas;
    ...
    void Start() {
        databaseNetworkManager = (FPS_Network_Manager) FPS_Network_Manager.singleton;
        databaseDataTranslator = gameObject.GetComponent<DataTranslator>();
    }
    ...
    IEnumerator LoginUser ()
    {
        ...
        if (response == "Success")
        {
            currentUser.username = playerUsername;
            currentUser.password = playerPassword;
            databaseNetworkManager.SetUser(currentUser);
    ...
    IEnumerator GetData ()
    {
        ...
        if (response == "Error")
        {
            ...
            if(LoginUICanvas.activeInHierarchy == true){
                ResetAllUIElements();
                playerUsername = "";
                playerPassword = "";
                loginParent.gameObject.SetActive(true);
        ...
        else
        {
            ...
            currentUser.total_kills = databaseDataTranslator.TextToData(currentUser.data, "kills");
            currentUser.total_deaths = databaseDataTranslator.TextToData(currentUser.data, "deaths");
    ...
    public void SetDataText(string dataText) {
        StartCoroutine(SetData(dataText));
    }
    ...
    public void ShowLogInUICanvas() {
        // LoginUICanvas.SetActive(true);
        StartCoroutine(ShowLogInUICanvasCoroutine());
    }

    public void HideLogInUICanvas() {
        LoginUICanvas.SetActive(false);
    }

    IEnumerator ShowLogInUICanvasCoroutine() {
